...
4. Before merging anything behavioral, ask for the independent review pass and check that the reviewer is not the author: `/codex:review --base main --background` for Claude-authored work (with the plugin), the `diff-reviewer` agent for Codex-authored work or when the plugin is absent. On native Windows, ask for the review *through the protocol* instead of typing that command yourself: the direct command hangs there, and the `auto` transport ships the same review as an inline task instead - see [Native Windows: reviews ship as inline tasks](#native-windows-reviews-ship-as-inline-tasks).
...
**Do not enable the plugin's automatic review gate as a default.** It registers a Stop hook that can loop Claude and Codex against each other and drain both quota pools. Review manually, at branch level: `/codex:review --base main --background` (replace `main` with your default branch if it differs; on native Windows the transport ships this same review as an inline task instead - next section).
