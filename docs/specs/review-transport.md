...
Revised 2026-07-09 for v0.3.1, after an independent Sol audit of the released v0.3.0 (4 findings, all verified against the plugin source, 1.0.6): the canonical inline-task dispatch moves from `/codex:rescue --background` to the plugin's companion CLI - `task --background --prompt-file <file> --json`. The slash command backgrounds the Claude-side subagent and strips `--background` before the runtime `task` call, so the dispatch never returns the `task-*` ID the deadline/cancel mechanics enforce against; and the rescue subagent re-materializes the prompt as a Bash argv on its way to the runtime, re-entering the ~32 KB Windows ceiling the transport exists to avoid - the round-2 claim that "the slash command carries the prompt through the plugin layer" was wrong one layer down. The job ID is now captured from the `--json` payload (a dispatch with no parseable ID fails over to diff-reviewer); read-only becomes structural (no `--write` flag passed) rather than prompt-interpreted; the companion dependency is a declared internal-interface coupling, re-verified on plugin updates; the temp prompt file is deleted the moment its dispatch returns, success or error (the queued job persists its own copy of the prompt); and the README's first-run and manual-review steps no longer point native-Windows users at typing the direct command the `auto` transport routes around.
...
3. **`inline-task`** delivers the review through the working task path: Claude computes the diff locally, writes the audit prompt (with an explicit no-commands / no-file-requests contract) to a temp file, dispatches it as a read-only background job on the plugin's companion CLI (`task --background --prompt-file <file> --json`; read-only is structural - no `--write` flag is passed), captures the `task-*` job ID from the JSON payload, deletes the temp file, and validates every finding against the repository afterwards. Never via `/codex:rescue`: its `--background` backgrounds the Claude-side subagent and is stripped before the runtime `task` call (no job ID returns), and the prompt re-enters Bash argv inside the rescue subagent. The companion CLI is an internal plugin interface - a declared coupling, re-verified on plugin updates (1.0.6, July 2026). Mechanics in the delegation-protocol skill §6.
...
9. **Timeout and cleanup are part of the transport.** Dispatch is background-first through the companion CLI; the `task-*` job ID is captured from the dispatch's `--json` payload (a dispatch with no parseable ID is a failed dispatch: fall back to `diff-reviewer`), the temp prompt file is deleted the moment the dispatch returns, and the deadline (15 minutes per chunk by default) is enforced by the orchestrator's own clock against the captured IDs - a bare blocking call cannot reach its own cancel instruction, so foreground dispatch is legal only under a harness-enforced hard timeout. On expiry: cancel the job (companion `cancel <job>` or `/codex:cancel <job>`), verify the review's own job IDs are no longer active, then fall back to `diff-reviewer` per the existing routing-table row. When the cancel runs through Git Bash, `MSYS_NO_PATHCONV=1` goes in that command's environment (MSYS otherwise mangles the underlying `taskkill /PID` into a path). A review that ends - success or failure - leaves none of its own jobs active; unrelated jobs are out of scope.
